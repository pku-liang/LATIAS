...
void Workload::set_type(const std::string& type) {
  if (type == "MATRIX" || type == "MAC") {
    type_ = Hardware::ArchTopology::LevelType::Matrix;
  } else if (type == "VECTOR" || type == "VEC") {
    type_ = Hardware::ArchTopology::LevelType::Vector;
  } else if (type == "BUFFER") {
    type_ = Hardware::ArchTopology::LevelType::Buffer;
  } else {
    TILEEXP_ERROR("Unknown workload type: " << type);
  }
}

std::string ParseWorkload(config::CompoundConfigNode config, problem::TileExp::Workload& workload) {
  std::string name;
  TILEEXP_ASSERT(config.lookupValue("name", name), "No name specified for an op"); // 这里的输入直接是ops下的，不包括io
  workload.set_name(name);
  workload.ParseShape(config); // 在这里parse具体的compute细节
  std::string ins, out;
  TILEEXP_ASSERT(config.lookupValue("ins", ins), "No ins specified for op " << name << ".");
  TILEEXP_ASSERT(config.lookupValue("out", out), "No out specified for op " << name << ".");

  workload.set_io(Split(ins), Split(out));

  std::string type;
  TILEEXP_ASSERT(config.lookupValue("type", type), "No type specified for op " << name << ".");
  workload.set_type(type);
  ...
  else if (config.exists("densities"))
  {
    ...
    for (unsigned i = 0; i < GetShape()->NumDataSpaces; i++){
      ynode["distribution"] = "fixed-structured";
      ynode["density"] = 1.0;
      const char *dummy = "dummy.yaml";
      density_config = config::CompoundConfigNode(nullptr, ynode, new config::CompoundConfig(dummy));
      auto density_specs = DensityDistributionFactory::ParseSpecs(density_config);
      densities[i]= DensityDistributionFactory::Construct(density_specs);
...
void ParseWorkloads(config::CompoundConfigNode config, Workloads& workloads) {
  if (config.exists("io")){
    ...
    std::string ins, outs;
    TILEEXP_ASSERT(io.lookupValue("ins", ins), "No ins property in problem::io");
    TILEEXP_ASSERT(io.lookupValue("outs", outs), "No outs property in problem::io");
    workloads.set_io(Split(ins), Split(outs));
  }
  else {
    TILEEXP_ERROR("No io found in problem.");  
  }
  ...
  if (config.exists("dimensions")) {
    ...
    TILEEXP_ERROR("no dimensions passed for the workloads.");
  }
  workloads.set_dims(dims); // set the dimensions to workload--shape, like FactorizedDimensionNameToID. build the relationship between MNK to 0,1,2
  
  if (config.exists("ops")) {
    auto ops = config.lookup("ops");
    for (int i = 0; i < ops.getLength(); ++i){
      std::shared_ptr<Workload> p_workload(new Workload(workloads));
      std::string name = problem::TileExp::ParseWorkload(ops[i], *p_workload);
      assert(workloads.add_workload(name, p_workload));
    }
  }
  else {
    std::shared_ptr<Workload> p_workload(new Workload(workloads));
    std::string name = problem::TileExp::ParseWorkload(config, *p_workload);
    assert(workloads.add_workload(name, p_workload));
  }
  ...
  if (config.exists("instance")) {
    ...
    for (auto dim: dims) {
      TILEEXP_ASSERT(factorized_bounds.exists(dim), "no instance passed for axis " << dim << ".");
      std::string _tmp;
      factorized_bounds.lookupValue(dim, _tmp);
      int bound;
      if (macros.exists(_tmp)) {
        TILEEXP_ASSERT(macros.lookupValue(_tmp, bound), 
  ...
  if (config.exists("coefficient")) { // usage? -- currently no use
...
bool Workloads::add_workload(const std::string & name, std::shared_ptr<Workload>& workload) {
  if (workloads_.count(name)) {
    TILEEXP_WARNING("Duplicate op named " << name << ". Drop all but the first.");
    return false;
  }
  ...
  for (auto& kv: shape_->DataSpaceIDToName) {
    ...
    if (common_shape_.DataSpaceNameToID.count(access_pattern_name)) {
      ...
      TILEEXP_ASSERT(common_shape_.DataSpaceOrder[id] == shape_->DataSpaceOrder.at(kv.first),
      "tensor " << access_pattern_name << " has mismatched order");
      continue;
    }
    auto& proj = shape_->Projections[kv.first]; // projection --> expression for each variable
...
void Workload::set_io(const std::vector<std::string>& ins, const std::vector<std::string>& outs) {
  ins_ = ins;
  TILEEXP_ASSERT(outs.size() == 1, "Currently, we only support op with single output");
  out_ = outs.front();
}
...
void Workload::Print(std::ostream& o) {
  ...
  o << "(";
  for (auto t: ins_) o << t << ",";
  o << ")->" << out_ << ", Compute Type: " << Hardware::ArchTopology::LevelType2String[type_];
  std::cout << std::endl;
}
